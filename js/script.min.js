...
(function(a){if(typeof define==="function"&&define.amd){define(["jquery"],a)}else{a(jQuery)}}(function(ak){ak.ui=ak.ui||{};var y=ak.ui.version="1.12.0";
/*!
 ...
 */
(function(){var aC,aF,aD=Math.max,aI=Math.abs,aG=Math.round,ax=/left|center|right/,aA=/top|center|bottom/,av=/[\+\-]\d+(\.[\d]+)?%?/,aE=/^\w+/,aw=/%$/,az=ak.fn.position;aF=function(){var aJ=ak("<div>").css("position","absolute").appendTo("body").offset({top:1.5,left:1.5}),aK=aJ.offset().top===1.5;aJ.remove();aF=function(){return aK};return aK};function aH(aL,aK,aJ){return[parseFloat(aL[0])*(aw.test(aL[0])?aK/100:1),parseFloat(aL[1])*(aw.test(aL[1])?aJ/100:1)]}function aB(aJ,aK){return parseInt(ak.css(aJ,aK),10)||0}function ay(aK){var aJ=aK[0];if(aJ.nodeType===9){return{width:aK.width(),height:aK.height(),offset:{top:0,left:0}}}if(ak.isWindow(aJ)){return{width:aK.width(),height:aK.height(),offset:{top:aK.scrollTop(),left:aK.scrollLeft()}}}if(aJ.preventDefault){return{width:0,height:0,offset:{top:aJ.pageY,left:aJ.pageX}}}return{width:aK.outerWidth(),height:aK.outerHeight(),offset:aK.offset()}}ak.position={scrollbarWidth:function(){if(aC!==undefined){return aC}var aK,aJ,aM=ak("<div style='display:block;position:absolute;width:50px;height:50px;overflow:hidden;'><div style='height:100px;width:auto;'></div></div>"),aL=aM.children()[0];ak("body").append(aM);aK=aL.offsetWidth;aM.css("overflow","scroll");aJ=aL.offsetWidth;if(aK===aJ){aJ=aM[0].clientWidth}aM.remove();return(aC=aK-aJ)},getScrollInfo:function(aN){var aM=aN.isWindow||aN.isDocument?"":aN.element.css("overflow-x"),aL=aN.isWindow||aN.isDocument?"":aN.element.css("overflow-y"),aK=aM==="scroll"||(aM==="auto"&&aN.width<aN.element[0].scrollWidth),aJ=aL==="scroll"||(aL==="auto"&&aN.height<aN.element[0].scrollHeight);return{width:aJ?ak.position.scrollbarWidth():0,height:aK?ak.position.scrollbarWidth():0}},getWithinInfo:function(aL){var aM=ak(aL||window),aJ=ak.isWindow(aM[0]),aN=!!aM[0]&&aM[0].nodeType===9,aK=!aJ&&!aN;return{element:aM,isWindow:aJ,isDocument:aN,offset:aK?ak(aL).offset():{left:0,top:0},scrollLeft:aM.scrollLeft(),scrollTop:aM.scrollTop(),width:aM.outerWidth(),height:aM.outerHeight()}}};ak.fn.position=function(aT){if(!aT||!aT.of){return az.apply(this,arguments)}aT=ak.extend({},aT);var aU,aQ,aO,aS,aN,aJ,aP=ak(aT.of),aM=ak.position.getWithinInfo(aT.within),aK=ak.position.getScrollInfo(aM),aR=(aT.collision||"flip").split(" "),aL={};aJ=ay(aP);if(aP[0].preventDefault){aT.at="left top"}aQ=aJ.width;aO=aJ.height;aS=aJ.offset;aN=ak.extend({},aS);ak.each(["my","at"],function(){var aX=(aT[this]||"").split(" "),aW,aV;if(aX.length===1){aX=ax.test(aX[0])?aX.concat(["center"]):aA.test(aX[0])?["center"].concat(aX):["center","center"]}aX[0]=ax.test(aX[0])?aX[0]:"center";aX[1]=aA.test(aX[1])?aX[1]:"center";aW=av.exec(aX[0]);aV=av.exec(aX[1]);aL[this]=[aW?aW[0]:0,aV?aV[0]:0];aT[this]=[aE.exec(aX[0])[0],aE.exec(aX[1])[0]]});if(aR.length===1){aR[1]=aR[0]}if(aT.at[0]==="right"){aN.left+=aQ}else{if(aT.at[0]==="center"){aN.left+=aQ/2}}if(aT.at[1]==="bottom"){aN.top+=aO}else{if(aT.at[1]==="center"){aN.top+=aO/2}}aU=aH(aL.at,aQ,aO);aN.left+=aU[0];aN.top+=aU[1];return this.each(function(){var aW,a5,aY=ak(this),a0=aY.outerWidth(),aX=aY.outerHeight(),aZ=aB(this,"marginLeft"),aV=aB(this,"marginTop"),a4=a0+aZ+aB(this,"marginRight")+aK.width,a3=aX+aV+aB(this,"marginBottom")+aK.height,a1=ak.extend({},aN),a2=aH(aL.my,aY.outerWidth(),aY.outerHeight());if(aT.my[0]==="right"){a1.left-=a0}else{if(aT.my[0]==="center"){a1.left-=a0/2}}if(aT.my[1]==="bottom"){a1.top-=aX}else{if(aT.my[1]==="center"){a1.top-=aX/2}}a1.left+=a2[0];a1.top+=a2[1];if(!aF()){a1.left=aG(a1.left);a1.top=aG(a1.top)}aW={marginLeft:aZ,marginTop:aV};ak.each(["left","top"],function(a7,a6){if(ak.ui.position[aR[a7]]){ak.ui.position[aR[a7]][a6](a1,{targetWidth:aQ,targetHeight:aO,elemWidth:a0,elemHeight:aX,collisionPosition:aW,collisionWidth:a4,collisionHeight:a3,offset:[aU[0]+a2[0],aU[1]+a2[1]],my:aT.my,at:aT.at,within:aM,elem:aY})}});if(aT.using){a5=function(a9){var bb=aS.left-a1.left,a8=bb+aQ-a0,ba=aS.top-a1.top,a7=ba+aO-aX,a6={target:{element:aP,left:aS.left,top:aS.top,width:aQ,height:aO},element:{element:aY,left:a1.left,top:a1.top,width:a0,height:aX},horizontal:a8<0?"left":bb>0?"right":"center",vertical:a7<0?"top":ba>0?"bottom":"middle"};if(aQ<a0&&aI(bb+a8)<aQ){a6.horizontal="center"}if(aO<aX&&aI(ba+a7)<aO){a6.vertical="middle"}if(aD(aI(bb),aI(a8))>aD(aI(ba),aI(a7))){a6.important="horizontal"}else{a6.important="vertical"}aT.using.call(this,a9,a6)}}aY.offset(ak.extend(a1,{using:a5}))})};ak.ui.position={fit:{left:function(aN,aM){var aL=aM.within,aP=aL.isWindow?aL.scrollLeft:aL.offset.left,aR=aL.width,aO=aN.left-aM.collisionPosition.marginLeft,aQ=aP-aO,aK=aO+aM.collisionWidth-aR-aP,aJ;if(aM.collisionWidth>aR){if(aQ>0&&aK<=0){aJ=aN.left+aQ+aM.collisionWidth-aR-aP;aN.left+=aQ-aJ}else{if(aK>0&&aQ<=0){aN.left=aP}else{if(aQ>aK){aN.left=aP+aR-aM.collisionWidth}else{aN.left=aP}}}}else{if(aQ>0){aN.left+=aQ}else{if(aK>0){aN.left-=aK}else{aN.left=aD(aN.left-aO,aN.left)}}}},top:function(aM,aL){var aK=aL.within,aQ=aK.isWindow?aK.scrollTop:aK.offset.top,aR=aL.within.height,aO=aM.top-aL.collisionPosition.marginTop,aP=aQ-aO,aN=aO+aL.collisionHeight-aR-aQ,aJ;if(aL.collisionHeight>aR){if(aP>0&&aN<=0){aJ=aM.top+aP+aL.collisionHeight-aR-aQ;aM.top+=aP-aJ}else{if(aN>0&&aP<=0){aM.top=aQ}else{if(aP>aN){aM.top=aQ+aR-aL.collisionHeight}else{aM.top=aQ}}}}else{if(aP>0){aM.top+=aP}else{if(aN>0){aM.top-=aN}else{aM.top=aD(aM.top-aO,aM.top)}}}}},flip:{left:function(aP,aO){var aN=aO.within,aT=aN.offset.left+aN.scrollLeft,aW=aN.width,aL=aN.isWindow?aN.scrollLeft:aN.offset.left,aQ=aP.left-aO.collisionPosition.marginLeft,aU=aQ-aL,aK=aQ+aO.collisionWidth-aW-aL,aS=aO.my[0]==="left"?-aO.elemWidth:aO.my[0]==="right"?aO.elemWidth:0,aV=aO.at[0]==="left"?aO.targetWidth:aO.at[0]==="right"?-aO.targetWidth:0,aM=-2*aO.offset[0],aJ,aR;if(aU<0){aJ=aP.left+aS+aV+aM+aO.collisionWidth-aW-aT;if(aJ<0||aJ<aI(aU)){aP.left+=aS+aV+aM}}else{if(aK>0){aR=aP.left-aO.collisionPosition.marginLeft+aS+aV+aM-aL;if(aR>0||aI(aR)<aK){aP.left+=aS+aV+aM}}}},top:function(aO,aN){var aM=aN.within,aV=aM.offset.top+aM.scrollTop,aW=aM.height,aJ=aM.isWindow?aM.scrollTop:aM.offset.top,aQ=aO.top-aN.collisionPosition.marginTop,aS=aQ-aJ,aP=aQ+aN.collisionHeight-aW-aJ,aT=aN.my[1]==="top",aR=aT?-aN.elemHeight:aN.my[1]==="bottom"?aN.elemHeight:0,aX=aN.at[1]==="top"?aN.targetHeight:aN.at[1]==="bottom"?-aN.targetHeight:0,aL=-2*aN.offset[1],aU,aK;if(aS<0){aK=aO.top+aR+aX+aL+aN.collisionHeight-aW-aV;if(aK<0||aK<aI(aS)){aO.top+=aR+aX+aL}}else{if(aP>0){aU=aO.top-aN.collisionPosition.marginTop+aR+aX+aL-aJ;if(aU>0||aI(aU)<aP){aO.top+=aR+aX+aL}}}}},flipfit:{left:function(){ak.ui.position.flip.left.apply(this,arguments);ak.ui.position.fit.left.apply(this,arguments)},top:function(){ak.ui.position.flip.top.apply(this,arguments);ak.ui.position.fit.top.apply(this,arguments)}}}})();var ah=ak.ui.position;
/*!
 ...
 */
;jQuery(document).ready(function(){var m=jQuery;var w=m("title");var f=m("#input");var v=m("#output");var s=m("#channel");var p=m("#topic");var j=m("#send-button");var h=m("#channel-button");var n=m("#whisper-button");var r=m("#user-button");var k=m("#list-button");var z=m("#option-button");var i=m("#info-button");var A=m("#logout-button");var q=m("#overlay");var e=m("#infobox");var l=m("#optionbox");var b=m("#connection");var t=[];var F=0;var d=0;m(document).keyup(function(H){if(H.keyCode==27){e.fadeOut(500,function(){q.fadeOut(500,function(){f.focus()})})}});f.focus();m("body").on("click","#output .link",function(){var H=m(this).text();m("#dialog").html("<p></p>");m("#dialog p").text(H);m("#dialog").dialog({title:"URL",resizable:false,buttons:{Open:function(){window.open(H);m(this).dialog("close");f.focus()},Cancel:function(){m(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})});m("body").on("click","#output .channel, .list-container .channel",function(){var H=m(this).text();m("#dialog").html("<p></p>");m("#dialog p").text(H);m("#dialog").dialog({title:"Channel",resizable:false,buttons:{Join:function(){o(H);m(this).dialog("close");f.focus()},Cancel:function(){m(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})});f.keyup(function(H){if(H.which==13){H.preventDefault();B()}else{if(H.keyCode==38){if(d<F){d++;f.val(t[F-d])}}else{if(H.keyCode==40){if(d>0){d--;f.val(t[F-d])}}}}});j.click(function(){B()});h.click(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getchannel"}}).done(function(H){if(H!==null){e.find("div").remove();e.append("<div><h2>Channel</h2></div>");m.each(H,function(I,J){e.append('<div class="join-container"><div class="join" title="'+J.topic+'">'+J.channel+'</div><span class="close" title="close"><i class="fa fa-close"></i></span></div>')});m(".join").on("click",function(){var J=m(this).text();var I=m(this).attr("title");x(J);w.text(J+" - Hades");s.text(J);p.text(I);e.fadeOut(500,function(){q.fadeOut(500,function(){f.focus()})})})}});q.fadeIn(500,function(){e.fadeIn(500)})});m("body").on("click",".box .close",function(){event.preventDefault();var I=m(this);var H=I.parent("div");G(H.text());H.remove()});m("body").on("click",".box .whois",function(){event.preventDefault();var I=m(this);var H=I.parent("div");u(H.text())});r.click(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getuser"}}).done(function(H){if(H!==null){e.find("div").remove();e.append("<div><h2>User at "+s.text()+"</h2></div>");m.each(H,function(I,J){e.append('<div class="whisper-container"><div class="whisper">'+J.username+'</div><span class="whois" title="whois"><i class="fa fa-question"></i></span></div>')})}});q.fadeIn(500,function(){e.fadeIn(500)})});n.click(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getwhisper"}}).done(function(H){if(H!==null){e.find("div").remove();e.append("<div><h2>Whisper User</h2></div>");m.each(H,function(I,J){e.append('<div class="whisper-container"><div class="whisper">'+J.channel+"</div></div>")});m(".whisper").on("click",function(){var I=m(this).text();x(I);w.text(I+" - Hades");s.text(I);p.text("");e.fadeOut(500,function(){q.fadeOut(500,function(){f.focus()})})})}});q.fadeIn(500,function(){e.fadeIn(500)})});z.click(function(){q.fadeIn(500,function(){l.fadeIn(500)})});k.click(function(){if(k.hasClass("fa-file-text")){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getchannellist"}}).done(function(H){if(H!==null){e.find("div").remove();e.append("<div><h2>Channel List</h2></div>");m.each(H,function(I,J){e.append('<div class="list-container"><div class="channel">'+J.channel+"</div></div>")})}});q.fadeIn(500,function(){e.fadeIn(500)})}else{if(k.hasClass("fa-file")){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/list"}})}}});i.click(function(){});A.click(function(){E()});function E(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"logout"}})}q.click(function(){var H=false;if(e.css("display")=="block"){H=e}if(l.css("display")=="block"){H=l}H.fadeOut(500,function(){q.fadeOut(500,function(){f.focus()})})});function B(){var H=f.val();if(H==""){return false}t[F]=H;F++;d=0;m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:H}}).done(function(I){if(I!==null){a(I)}});f.val("")}function o(H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/join "+H}}).done(function(I){if(I!==null){a(I)}})}function G(H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/part "+H}}).done(function(I){if(I!==null){}})}function u(H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/whois "+H}}).done(function(I){if(I!==null){a(I)}})}function D(H){v.append("<p>"+H+"</p>");c()}function c(){if(autoscroll){m(".output").stop().animate({scrollTop:m(".scrollto")[0].offsetTop},1000)}}function x(H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setchannel",channel:H}});v.find("p").remove()}function g(H){if(H!==""){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"gettopic",channel:H}}).done(function(I){if(I!==null&&I[0]!==undefined&&I[0].topic!==undefined){p.html(I[0].topic)}else{p.html("")}})}}function a(H){if(H.action==="join"){var I=H.channel[0];x(I);w.text(I+" - Hades");s.text(I);p.text("")}if(H.action==="logout"){E()}if(H.type==="status"){y(H)}}function y(H){if(H.status===undefined){return false}if(parseInt(H.status)===323){k.removeClass("fa-file");k.addClass("fa-file-text");return null}m("#dialog").html("<p></p>");m("#dialog p").html(H.text);m("#dialog").dialog({title:H.status,resizable:false,buttons:{Ok:function(){m(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}});if(H.status==="470"){var I=H.data.forwarding;x(I);w.text(I+" - Hades");s.text(I)}}function C(H){m("#dialog").html("<p></p>");m("#dialog p").text(H.text);m("#dialog").dialog({title:H.status,resizable:false,buttons:{Join:function(){o(H.data.channel);m(this).dialog("close");f.focus()},Ignore:function(){m(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})}setInterval(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getstatus"}}).done(function(H){if(H!==null&&H.status!==undefined){if(H.status=="INVITE"){C(H)}else{y(H)}}})},5000);m("body").on("click","#output .lock",function(){var H=m(this);if(H.hasClass("fa-lock")){H.removeClass("fa-lock");H.addClass("fa-unlock");H.parent("p").find(".text").css("display","");H.parent("p").find(".crypt").css("display","none")}else{if(H.hasClass("fa-unlock")){H.removeClass("fa-unlock");H.addClass("fa-lock");H.parent("p").find(".text").css("display","none");H.parent("p").find(".crypt").css("display","")}}});setInterval(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getoutput"}}).done(function(H){if(H!==null){if(H.loggedin===false){location.href="login.php"}else{m.each(H,function(J,L){var K=new Date(L.time);var I='[<span class="time" title="'+K.toLocaleTimeString()+" / "+K.toLocaleDateString()+'">'+K.toLocaleTimeString()+"</span>] ";if(L.crypt!=undefined){I+='<i class="lock fa fa-lock"></i> '}if(L.action==1){I+='<span class="action">'+L.name+" "+L.text+"</span>"}else{I+='&lt;<span class="user" title="'+L.name+'">'+L.name+'</span>&gt; <span class="text"'+(L.crypt!==undefined?' style="display: none;"':"")+">"+L.text+"</span>"}if(L.crypt!==undefined){I+='<span class="crypt">'+L.crypt+"</span>"}D(I);c()})}}})},2000);setInterval(function(){g(m("#channel").text())},10000);setInterval(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"isrunning"}}).done(function(H){if(H===false){b.children(".fa-flash").css("display","none");b.children(".fa-refresh").css("display","")}else{b.children(".fa-flash").css("display","");b.children(".fa-refresh").css("display","none")}})},12000);b.on("click",".fa-refresh",function(){location.reload(true)});m(window).resize(function(){c()});g(m("#channel").text());m(window).bind("beforeunload",function(){if(m(".fa-refresh").css("display")==="none"){return""}});m("body").on("change","#theme-default, #theme-dark, #theme-light",function(){var H=m(this);if(H.prop("checked")===true){m("body").attr("class","").addClass("theme-"+H.attr("value"))}});m("body").on("change","#autoscroll-enable, #autoscroll-disable",function(){var H=m(this);if(H.prop("checked")===true){autoscroll=H.attr("value")==="true"}});m(".input").hover(function(){var I=m(".input > .menu");I.css("height","auto");var H=I.height()+20;I.css("height","0")},function(){var H=m(".input > .menu")})});var autoscroll=true;