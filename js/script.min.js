 ...
 */
;jQuery(document).ready(function(){var l=jQuery;var u=l("title");var f=l("#input");var t=l("#output");var r=l("#channel");var o=l("#topic");var j=l("#send-button");var h=l("#channel-button");var m=l("#whisper-button");var q=l("#user-button");var x=l("#option-button");var i=l("#info-button");var y=l("#logout-button");var p=l("#overlay");var e=l("#infobox");var k=l("#optionbox");var b=l("#connection");var s=[];var D=0;var d=0;l(document).keyup(function(F){if(F.keyCode==27){e.fadeOut(500,function(){p.fadeOut(500,function(){f.focus()})})}});f.focus();l("body").on("click","#output .link",function(){var F=l(this).text();l("#dialog").html("<p></p>");l("#dialog p").text(F);l("#dialog").dialog({title:"URL",resizable:false,buttons:{Open:function(){window.open(F);l(this).dialog("close");f.focus()},Cancel:function(){l(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})});l("body").on("click","#output .channel",function(){var F=l(this).text();l("#dialog").html("<p></p>");l("#dialog p").text(F);l("#dialog").dialog({title:"Channel",resizable:false,buttons:{Join:function(){n(F);l(this).dialog("close");f.focus()},Cancel:function(){l(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})});f.keyup(function(F){if(F.which==13){F.preventDefault();z()}else{if(F.keyCode==38){if(d<D){d++;f.val(s[D-d])}}else{if(F.keyCode==40){if(d>0){d--;f.val(s[D-d])}}}}});j.click(function(){z()});h.click(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getchannel"}}).done(function(F){if(F!==null){e.find("div").remove();e.append("<div><h2>Channel</h2></div>");l.each(F,function(G,H){e.append('<div class="join-container"><div class="join" title="'+H.topic+'">'+H.channel+'</div><span class="close" title="close"><i class="fa fa-close"></i></span></div>')});l(".join").on("click",function(){var H=l(this).text();var G=l(this).attr("title");v(H);u.text(H+" - Hades");r.text(H);o.text(G);e.fadeOut(500,function(){p.fadeOut(500,function(){f.focus()})})})}});p.fadeIn(500,function(){e.fadeIn(500)})});l("body").on("click",".infobox .close",function(){event.preventDefault();var G=l(this);var F=G.parent("div");E(F.text());F.remove()});q.click(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getuser"}}).done(function(F){if(F!==null){e.find("div").remove();e.append("<div><h2>User at "+r.text()+"</h2></div>");l.each(F,function(G,H){e.append('<div class="whisper-container"><div class="whisper">'+H.username+"</div></div>")})}});p.fadeIn(500,function(){e.fadeIn(500)})});m.click(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getwhisper"}}).done(function(F){if(F!==null){e.find("div").remove();e.append("<div><h2>Whisper User</h2></div>");l.each(F,function(G,H){e.append('<div class="whisper-container"><div class="whisper">'+H.channel+"</div></div>")});l(".whisper").on("click",function(){var G=l(this).text();v(G);u.text(G+" - Hades");r.text(G);o.text("");e.fadeOut(500,function(){p.fadeOut(500,function(){f.focus()})})})}});p.fadeIn(500,function(){e.fadeIn(500)})});x.click(function(){p.fadeIn(500,function(){k.fadeIn(500)})});x.click(function(){});i.click(function(){});y.click(function(){C()});function C(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"logout"}})}p.click(function(){var F=false;if(e.css("display")=="block"){F=e}if(k.css("display")=="block"){F=k}F.fadeOut(500,function(){p.fadeOut(500,function(){f.focus()})})});function z(){var F=f.val();if(F==""){return false}s[D]=F;D++;d=0;l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:F}}).done(function(G){if(G!==null){a(G)}});f.val("")}function n(F){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/join "+F}}).done(function(G){if(G!==null){a(G)}})}function E(F){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/part "+F}}).done(function(G){if(G!==null){}})}function B(F){t.append("<p>"+F+"</p>");c()}function c(){if(autoscroll){l(".output").stop().animate({scrollTop:l(".scrollto")[0].offsetTop},1000)}}function v(F){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setchannel",channel:F}});t.find("p").remove()}function g(F){if(F!==""){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"gettopic",channel:F}}).done(function(G){if(G!==null&&G[0]!==undefined&&G[0].topic!==undefined){o.html(G[0].topic)}else{o.html("")}})}}function a(F){if(F.action==="join"){var G=F.channel[0];v(G);u.text(G+" - Hades");r.text(G);o.text("")}if(F.action==="logout"){C()}if(F.type==="status"){w(F)}}function w(F){if(F.status===undefined){return false}l("#dialog").html("<p></p>");l("#dialog p").text(F.text);l("#dialog").dialog({title:F.status,resizable:false,buttons:{Ok:function(){l(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})}function A(F){l("#dialog").html("<p></p>");l("#dialog p").text(F.text);l("#dialog").dialog({title:F.status,resizable:false,buttons:{Join:function(){n(F.data.channel);l(this).dialog("close");f.focus()},Ignore:function(){l(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})}setInterval(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getstatus"}}).done(function(F){if(F!==null&&F.status!==undefined){if(F.status=="INVITE"){A(F)}else{w(F)}}})},3000);l("body").on("click","#output .lock",function(){var F=l(this);if(F.hasClass("fa-lock")){F.removeClass("fa-lock");F.addClass("fa-unlock");F.parent("p").find(".text").css("display","");F.parent("p").find(".crypt").css("display","none")}else{if(F.hasClass("fa-unlock")){F.removeClass("fa-unlock");F.addClass("fa-lock");F.parent("p").find(".text").css("display","none");F.parent("p").find(".crypt").css("display","")}}});setInterval(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getoutput"}}).done(function(F){if(F!==null){if(F.loggedin===false){location.href="login.php"}else{l.each(F,function(H,J){var I=new Date(J.time);var G='[<span class="time" title="'+I.toLocaleTimeString()+" / "+I.toLocaleDateString()+'">'+I.toLocaleTimeString()+"</span>] ";if(J.crypt!=undefined){G+='<i class="lock fa fa-lock"></i> '}if(J.action==1){G+='<span class="action">'+J.name+" "+J.text+"</span>"}else{G+='&lt;<span class="user" title="'+J.name+'">'+J.name+'</span>&gt; <span class="text"'+(J.crypt!==undefined?' style="display: none;"':"")+">"+J.text+"</span>"}if(J.crypt!==undefined){G+='<span class="crypt">'+J.crypt+"</span>"}B(G);c()})}}})},2000);setInterval(function(){g(l("#channel").text())},10000);setInterval(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"isrunning"}}).done(function(F){if(F===false){b.children(".fa-flash").css("display","none");b.children(".fa-refresh").css("display","")}else{b.children(".fa-flash").css("display","");b.children(".fa-refresh").css("display","none")}})},12000);b.on("click",".fa-refresh",function(){location.reload(true)});l(window).resize(function(){c()});g(l("#channel").text());l(window).bind("beforeunload",function(){return""});l("body").on("change","#theme-default, #theme-dark, #theme-light",function(){var F=l(this);if(F.prop("checked")===true){l("body").attr("class","").addClass("theme-"+F.attr("value"))}});l("body").on("change","#autoscroll-enable, #autoscroll-disable",function(){var F=l(this);if(F.prop("checked")===true){autoscroll=F.attr("value")==="true"}});l(".input").hover(function(){var G=l(".input > .menu");G.css("height","auto");var F=G.height()+20;G.css("height","0")},function(){var F=l(".input > .menu")})});var autoscroll=true;