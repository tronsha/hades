 ...
 */
;jQuery(document).ready(function(){var l=jQuery;var v=l("title");var f=l("#input");var u=l("#output");var r=l("#channel");var o=l("#topic");var j=l("#send-button");var h=l("#channel-button");var m=l("#whisper-button");var q=l("#user-button");var y=l("#option-button");var i=l("#info-button");var z=l("#logout-button");var p=l("#overlay");var e=l("#infobox");var k=l("#optionbox");var b=l("#connection");var s=[];var E=0;var d=0;l(document).keyup(function(G){if(G.keyCode==27){e.fadeOut(500,function(){p.fadeOut(500,function(){f.focus()})})}});f.focus();l("body").on("click","#output .link",function(){var G=l(this).text();l("#dialog").html("<p></p>");l("#dialog p").text(G);l("#dialog").dialog({title:"URL",resizable:false,buttons:{Open:function(){window.open(G);l(this).dialog("close");f.focus()},Cancel:function(){l(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})});l("body").on("click","#output .channel",function(){var G=l(this).text();l("#dialog").html("<p></p>");l("#dialog p").text(G);l("#dialog").dialog({title:"Channel",resizable:false,buttons:{Join:function(){n(G);l(this).dialog("close");f.focus()},Cancel:function(){l(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})});f.keyup(function(G){if(G.which==13){G.preventDefault();A()}else{if(G.keyCode==38){if(d<E){d++;f.val(s[E-d])}}else{if(G.keyCode==40){if(d>0){d--;f.val(s[E-d])}}}}});j.click(function(){A()});h.click(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getchannel"}}).done(function(G){if(G!==null){e.find("div").remove();e.append("<div><h2>Channel</h2></div>");l.each(G,function(H,I){e.append('<div class="join-container"><div class="join" title="'+I.topic+'">'+I.channel+'</div><span class="close" title="close"><i class="fa fa-close"></i></span></div>')});l(".join").on("click",function(){var I=l(this).text();var H=l(this).attr("title");w(I);v.text(I+" - Hades");r.text(I);o.text(H);e.fadeOut(500,function(){p.fadeOut(500,function(){f.focus()})})})}});p.fadeIn(500,function(){e.fadeIn(500)})});l("body").on("click",".box .close",function(){event.preventDefault();var H=l(this);var G=H.parent("div");F(G.text());G.remove()});l("body").on("click",".box .whois",function(){event.preventDefault();var H=l(this);var G=H.parent("div");t(G.text())});q.click(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getuser"}}).done(function(G){if(G!==null){e.find("div").remove();e.append("<div><h2>User at "+r.text()+"</h2></div>");l.each(G,function(H,I){e.append('<div class="whisper-container"><div class="whisper">'+I.username+'</div><span class="whois" title="whois"><i class="fa fa-question"></i></span></div>')})}});p.fadeIn(500,function(){e.fadeIn(500)})});m.click(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getwhisper"}}).done(function(G){if(G!==null){e.find("div").remove();e.append("<div><h2>Whisper User</h2></div>");l.each(G,function(H,I){e.append('<div class="whisper-container"><div class="whisper">'+I.channel+"</div></div>")});l(".whisper").on("click",function(){var H=l(this).text();w(H);v.text(H+" - Hades");r.text(H);o.text("");e.fadeOut(500,function(){p.fadeOut(500,function(){f.focus()})})})}});p.fadeIn(500,function(){e.fadeIn(500)})});y.click(function(){p.fadeIn(500,function(){k.fadeIn(500)})});i.click(function(){});z.click(function(){D()});function D(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"logout"}})}p.click(function(){var G=false;if(e.css("display")=="block"){G=e}if(k.css("display")=="block"){G=k}G.fadeOut(500,function(){p.fadeOut(500,function(){f.focus()})})});function A(){var G=f.val();if(G==""){return false}s[E]=G;E++;d=0;l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:G}}).done(function(H){if(H!==null){a(H)}});f.val("")}function n(G){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/join "+G}}).done(function(H){if(H!==null){a(H)}})}function F(G){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/part "+G}}).done(function(H){if(H!==null){}})}function t(G){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/whois "+G}}).done(function(H){if(H!==null){a(H)}})}function C(G){u.append("<p>"+G+"</p>");c()}function c(){if(autoscroll){l(".output").stop().animate({scrollTop:l(".scrollto")[0].offsetTop},1000)}}function w(G){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setchannel",channel:G}});u.find("p").remove()}function g(G){if(G!==""){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"gettopic",channel:G}}).done(function(H){if(H!==null&&H[0]!==undefined&&H[0].topic!==undefined){o.html(H[0].topic)}else{o.html("")}})}}function a(G){if(G.action==="join"){var H=G.channel[0];w(H);v.text(H+" - Hades");r.text(H);o.text("")}if(G.action==="logout"){D()}if(G.type==="status"){x(G)}}function x(G){if(G.status===undefined){return false}if(parseInt(G.status)===323){jQuery("#list-button").css("display","inline");return null}l("#dialog").html("<p></p>");l("#dialog p").html(G.text);l("#dialog").dialog({title:G.status,resizable:false,buttons:{Ok:function(){l(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}});if(G.status==="470"){var H=G.data.forwarding;w(H);v.text(H+" - Hades");r.text(H)}}function B(G){l("#dialog").html("<p></p>");l("#dialog p").text(G.text);l("#dialog").dialog({title:G.status,resizable:false,buttons:{Join:function(){n(G.data.channel);l(this).dialog("close");f.focus()},Ignore:function(){l(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})}setInterval(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getstatus"}}).done(function(G){if(G!==null&&G.status!==undefined){if(G.status=="INVITE"){B(G)}else{x(G)}}})},5000);l("body").on("click","#output .lock",function(){var G=l(this);if(G.hasClass("fa-lock")){G.removeClass("fa-lock");G.addClass("fa-unlock");G.parent("p").find(".text").css("display","");G.parent("p").find(".crypt").css("display","none")}else{if(G.hasClass("fa-unlock")){G.removeClass("fa-unlock");G.addClass("fa-lock");G.parent("p").find(".text").css("display","none");G.parent("p").find(".crypt").css("display","")}}});setInterval(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getoutput"}}).done(function(G){if(G!==null){if(G.loggedin===false){location.href="login.php"}else{l.each(G,function(I,K){var J=new Date(K.time);var H='[<span class="time" title="'+J.toLocaleTimeString()+" / "+J.toLocaleDateString()+'">'+J.toLocaleTimeString()+"</span>] ";if(K.crypt!=undefined){H+='<i class="lock fa fa-lock"></i> '}if(K.action==1){H+='<span class="action">'+K.name+" "+K.text+"</span>"}else{H+='&lt;<span class="user" title="'+K.name+'">'+K.name+'</span>&gt; <span class="text"'+(K.crypt!==undefined?' style="display: none;"':"")+">"+K.text+"</span>"}if(K.crypt!==undefined){H+='<span class="crypt">'+K.crypt+"</span>"}C(H);c()})}}})},2000);setInterval(function(){g(l("#channel").text())},10000);setInterval(function(){l.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"isrunning"}}).done(function(G){if(G===false){b.children(".fa-flash").css("display","none");b.children(".fa-refresh").css("display","")}else{b.children(".fa-flash").css("display","");b.children(".fa-refresh").css("display","none")}})},12000);b.on("click",".fa-refresh",function(){location.reload(true)});l(window).resize(function(){c()});g(l("#channel").text());l(window).bind("beforeunload",function(){if(l(".fa-refresh").css("display")==="none"){return""}});l("body").on("change","#theme-default, #theme-dark, #theme-light",function(){var G=l(this);if(G.prop("checked")===true){l("body").attr("class","").addClass("theme-"+G.attr("value"))}});l("body").on("change","#autoscroll-enable, #autoscroll-disable",function(){var G=l(this);if(G.prop("checked")===true){autoscroll=G.attr("value")==="true"}});l(".input").hover(function(){var H=l(".input > .menu");H.css("height","auto");var G=H.height()+20;H.css("height","0")},function(){var G=l(".input > .menu")})});var autoscroll=true;