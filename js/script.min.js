 ...
 */
;jQuery(document).ready(function(){var m=jQuery;var w=m("title");var f=m("#input");var v=m("#output");var s=m("#channel");var p=m("#topic");var j=m("#send-button");var h=m("#channel-button");var n=m("#whisper-button");var r=m("#user-button");var k=m("#list-button");var z=m("#option-button");var i=m("#info-button");var A=m("#logout-button");var q=m("#overlay");var e=m("#infobox");var l=m("#optionbox");var b=m("#connection");var t=[];var F=0;var d=0;m(document).on("keyup",function(H){if(27===H.keyCode){e.fadeOut(500,function(){q.fadeOut(500,function(){f.focus()})})}});f.focus();m("body").on("click","#output .link",function(){var H=m(this).text();m("#dialog").html("<p></p>");m("#dialog p").text(H);m("#dialog").dialog({title:"URL",resizable:false,buttons:{Open:function(){window.open(H);m(this).dialog("close");f.focus()},Cancel:function(){m(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})});m("body").on("click","#output .channel, .list-container .channel",function(){var H=m(this).text();m("#dialog").html("<p></p>");m("#dialog p").text(H);m("#dialog").dialog({title:"Channel",resizable:false,buttons:{Join:function(){o(H);m(this).dialog("close");f.focus()},Cancel:function(){m(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})});f.on("keyup",function(H){if(13===H.which){H.preventDefault();B()}else{if(38===H.keyCode){if(d<F){d++;f.val(t[F-d])}}else{if(40===H.keyCode){if(d>0){d--;f.val(t[F-d])}}}}});j.on("click",function(){B()});h.on("click",function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getchannel"}}).done(function(H){if(null!==H){e.find("div").remove();e.append("<div><h2>Channel</h2></div>");m.each(H,function(I,J){e.append('<div class="join-container"><div class="join" title="'+J.topic+'">'+J.channel+'</div><span class="close" title="close"><i class="fa fa-close"></i></span></div>')});m(".join").on("click",function(){var J=m(this).text();var I=m(this).attr("title");x(J);w.text(J+" - Hades");s.text(J);p.text(I);e.fadeOut(500,function(){q.fadeOut(500,function(){f.focus()})})})}});q.fadeIn(500,function(){e.fadeIn(500)})});m("body").on("click",".box .close",function(){event.preventDefault();var I=m(this);var H=I.parent("div");G(H.text());H.remove()});m("body").on("click",".box .whois",function(){event.preventDefault();var I=m(this);var H=I.parent("div");u(H.text())});r.on("click",function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getuser"}}).done(function(H){if(null!==H){e.find("div").remove();e.append("<div><h2>User at "+s.text()+"</h2></div>");m.each(H,function(I,J){e.append('<div class="whisper-container"><div class="whisper">'+J.username+'</div><span class="whois" title="whois"><i class="fa fa-question"></i></span></div>')})}});q.fadeIn(500,function(){e.fadeIn(500)})});n.on("click",function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getwhisper"}}).done(function(H){if(null!==H){e.find("div").remove();e.append("<div><h2>Whisper User</h2></div>");m.each(H,function(I,J){e.append('<div class="whisper-container"><div class="whisper">'+J.channel+"</div></div>")});m(".whisper").on("click",function(){var I=m(this).text();x(I);w.text(I+" - Hades");s.text(I);p.text("");e.fadeOut(500,function(){q.fadeOut(500,function(){f.focus()})})})}});q.fadeIn(500,function(){e.fadeIn(500)})});z.on("click",function(){q.fadeIn(500,function(){l.fadeIn(500)})});k.on("click",function(){if(k.hasClass("disabled")){return false}if(k.hasClass("fa-file-text")){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getchannellist"}}).done(function(H){if(null!==H){e.find("div").remove();e.append("<div><h2>Channel List</h2></div>");m.each(H,function(I,J){e.append('<div class="list-container"><div class="channel">'+J.channel+"</div></div>")})}});q.fadeIn(500,function(){e.fadeIn(500)})}else{if(k.hasClass("fa-file")){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/list"}});k.addClass("disabled")}}return true});i.on("click",function(){});A.on("click",function(){E()});function E(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"logout"}})}q.on("click",function(){var H=false;if("block"===e.css("display")){H=e}if("block"===l.css("display")){H=l}H.fadeOut(500,function(){q.fadeOut(500,function(){f.focus()})})});function B(){var H=f.val();if(""===H){return false}t[F]=H;F++;d=0;m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:H}}).done(function(I){if(null!==I){a(I)}});f.val("")}function o(H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/join "+H}}).done(function(I){if(null!==I){a(I)}})}function G(H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/part "+H}}).done(function(I){if(null!==I){}})}function u(H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setinput",text:"/whois "+H}}).done(function(I){if(null!==I){a(I)}})}function D(H){v.append("<p>"+H+"</p>");c()}function c(){if(autoscroll){m(".output").stop().animate({scrollTop:m(".scrollto")[0].offsetTop},1000)}}function x(H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"setchannel",channel:H}});v.find("p").remove()}function g(H){if(""!==H){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"gettopic",channel:H}}).done(function(I){if(null!==I&&undefined!==I[0]&&undefined!==I[0].topic){p.html(I[0].topic)}else{p.html("")}})}}function a(H){if("join"===H.action){var I=H.channel[0];x(I);w.text(I+" - Hades");s.text(I);p.text("")}if("logout"===H.action){E()}if("status"===H.type){y(H)}}function y(H){if(undefined===H.status){return false}if(323===parseInt(H.status)){k.removeClass("fa-file");k.addClass("fa-file-text");k.removeClass("disabled");setTimeout(function(){k.removeClass("fa-file-text");k.addClass("fa-file")},3600000);return null}m("#dialog").html("<p></p>");m("#dialog p").html(H.text);m("#dialog").dialog({title:H.status,resizable:false,buttons:{Ok:function(){m(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}});if(470===parseInt(H.status)){var I=H.data.forwarding;x(I);w.text(I+" - Hades");s.text(I)}return true}function C(H){m("#dialog").html("<p></p>");m("#dialog p").text(H.text);m("#dialog").dialog({title:H.status,resizable:false,buttons:{Join:function(){o(H.data.channel);m(this).dialog("close");f.focus()},Ignore:function(){m(this).dialog("close");f.focus()}},show:{effect:"fade",duration:500},hide:{effect:"fade",duration:500}})}setInterval(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getstatus"}}).done(function(H){if(null!==H&&undefined!==H.status){if("INVITE"===H.status){C(H)}else{y(H)}}})},5000);m("body").on("click","#output .lock",function(){var H=m(this);if(H.hasClass("fa-lock")){H.removeClass("fa-lock");H.addClass("fa-unlock");H.parent("p").find(".text").css("display","");H.parent("p").find(".crypt").css("display","none")}else{if(H.hasClass("fa-unlock")){H.removeClass("fa-unlock");H.addClass("fa-lock");H.parent("p").find(".text").css("display","none");H.parent("p").find(".crypt").css("display","")}}});setInterval(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"getoutput"}}).done(function(H){if(null!==H){if(false===H.loggedin){location.href="login.php"}else{m.each(H,function(J,L){var K=new Date(L.time);var I='[<span class="time" title="'+K.toLocaleTimeString()+" / "+K.toLocaleDateString()+'">'+K.toLocaleTimeString()+"</span>] ";if(undefined!==L.crypt){I+='<i class="lock fa fa-lock"></i> '}if(1===parseInt(L.action)){I+='<span class="action">'+L.name+" "+L.text+"</span>"}else{I+='&lt;<span class="user" title="'+L.name+'">'+L.name+'</span>&gt; <span class="text"'+(undefined!==L.crypt?' style="display: none;"':"")+">"+L.text+"</span>"}if(undefined!==L.crypt){I+='<span class="crypt">'+L.crypt+"</span>"}D(I);c()})}}})},3000);setInterval(function(){g(m("#channel").text())},10000);setInterval(function(){m.ajax({url:"ajax.php",type:"POST",dataType:"json",data:{action:"isrunning"}}).done(function(H){if(false===H){b.children(".fa-flash").css("display","none");b.children(".fa-refresh").css("display","")}else{b.children(".fa-flash").css("display","");b.children(".fa-refresh").css("display","none")}})},12000);b.on("click",".fa-refresh",function(){location.reload(true)});m(window).resize(function(){c()});g(m("#channel").text());m(window).on("beforeunload",function(){if("none"===m(".fa-refresh").css("display")){return""}});m("body").on("change","#theme-default, #theme-dark, #theme-light",function(){var H=m(this);if(true===H.prop("checked")){m("body").attr("class","").addClass("theme-"+H.attr("value"))}});m("body").on("change","#autoscroll-enable, #autoscroll-disable",function(){var H=m(this);if(true===H.prop("checked")){autoscroll="true"===H.attr("value")}});m(".input").hover(function(){var I=m(".input > .menu");I.css("height","auto");var H=I.height()+20;I.css("height","0")},function(){var H=m(".input > .menu")})});var autoscroll=true;